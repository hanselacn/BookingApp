...
func (rtr *router) Route() *routerkit.Router {
	...
	// //CORS HEADER
	// ch := handlers.CORS(handlers.AllowedOrigins(["http://localhost:8080"]))
	...
	root.HandleFunc("/user/grant/{userID}", rtr.handle(
		handler.HttpRequest,
		grantAdmin,
	)).Methods(http.MethodPut)

	demoteUser := ucase.NewUpdateUser(userRepository)
	root.HandleFunc("/user/demote/{userID}", rtr.handle(
		handler.HttpRequest,
		demoteUser,
	)).Methods(http.MethodPut)
	...
	createBooking := ucase.NewCreateBooking(bookingRepository)
	...
	createRoom := ucase.NewCreateRoom(roomRepository)
	...
	deleteRoom := ucase.NewDeleteRoom(roomRepository)
	root.HandleFunc("/room/{roomID}", rtr.handle(
		handler.HttpRequest,
		deleteRoom,
	)).Methods(http.MethodDelete)

	//DAY
	createDay := ucase.NewCreateDay(dayRepository)
